...
from .parser import ObsDoc, get_wikilinks, clean_links
...
class ColorclassProcessor:
    ...
    def _load_config(self, config_path: str | None) -> DictConfig:
        ...
        if config_path:
            ...
            if config_path_obj.exists():
                ...
                return OmegaConf.create(merged) if not isinstance(merged, DictConfig) else merged
            else:
    ...
    def process_vault(
        ...
        if not dry_run:
            modified_count = self._apply_assignments(corpus, vault_path_obj, assignments)
    ...
    def _run_networkx_algorithm(self, graph: nx.Graph, algorithm: str) -> list[set[str]]:
    ...
    def _recursive_kernighan_lin(self, graph: nx.Graph, params: dict[str, Any], max_depth: int = 4) -> list[set[str]]:
    ...
    def _process_communities(self, communities: list[set[str]], graph: nx.Graph) -> dict[str, str]:
    ...
    def _add_colorclass_tag(self, doc: ObsDoc, vault_path: Path, colorclass_tag: str) -> bool:
        ...
        post.metadata['tags'] = [
            tag for tag in post.metadata['tags'] 
    ...
    def analyze_community_structure(self, vault_path: str, algorithm: str | None = None) -> dict[str, Any]:
